...
object Problem2 {
  ...
  def order( node:Node ) {
    ...
    def preorder(node: Option[Node], id: Int): Int = {
      node match{
        case Some(node) => {
          ...
          return preorder(node.right, preorder(node.left, id + 1) + 1)
        }
        case _ => return id -1
      }
    }

    def postorder(node: Option[Node], id: Int): Int = {
      node match{
        case Some(node) => {
          node.ord = (node.ord._1, postorder(node.right, postorder(node.left, id)))
          return node.ord._2 + 1
        }
        case _ => return id
      }
    }
    preorder(Option(node), 0)
    postorder(Option(node), 0)
...
object Problem3 {
  def main( args:Array[String] ) = {
    ...
    println(list.foldLeft(empty)((a: String, b:String) => 
      if(b > a) b else a))



    println(list.foldLeft(List[String]())((a: List[String], b:String) =>
      if(!a.contains(b)) b :: a else a).reverse)


    //Based recursive idea off of http://stackoverflow.com/questions/14550537/run-length-encoding-using-scala
    //Learned about permutation of the tail / last element to add to the list from https://oldfashionedsoftware.com/2009/07/30/lots-and-lots-of-foldleft-examples/
    //
    println(list.foldLeft(List[(String, Int)]())((a: List[(String, Int)], b:String) => {
      a match {
        case (string: String, count: Int) ::tail => if(string == b) (string, count + 1) :: tail else (b,1) :: a
        //TODO learn how to screen for Nil in a list of tuples
        case _ => (b,1) :: a
      }
    }).reverse)
    ...
    ()
    //println(list.foldLeft(List[String]())(a,b: String))

    //println(list.foldLeft(Set[String](""))( (a: Set[String], b:String) => if(b > a.head) a - a.head + b)  )
    ...
    /*println(list.foldLeft(empty)( (a: String, b: String) => {
        Option(b) match{
          case Some(b: String) if(b > a) => b
        }
      }))*/
    //println(list.foldLeft(Seq[String](args(0))((a: Seq[String],b: String) => if(b > a(0)) a - a(0) + b)))

    //val lrg = list.foldLeft(List(list(0)))( (a,b) => if(b > a(0)) (a - a(0)) + b )
    //println(lrg)

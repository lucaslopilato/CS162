...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  def map[B](f: A => B): MyList[B] = {
    if(this.isEmpty) MyList[B]() else MyCons[B](f(x), xs.map[B](f))
  }


  def flatMap[B](f: A => MyList[B]): MyList[B] = {
    if(this.isEmpty) MyList[B]() else f(x).append(xs.flatMap[B](f))
  }

  def filter(pred: A => Boolean): MyList[A] = {
    val t = pred(x)
    xs match{
      case MyNil() => if(t) MyList[A](x) else MyNil()
      case _ => {
        if(t) MyCons[A](x, xs.filter(pred))   
        else xs.filter(pred)
      }
    }
  }
  ...
  def append(other: MyList[A]): MyList[A] = {
    xs match{
      case MyNil() => MyCons[A](x, other)
      case _ => MyCons[A](x,xs.append(other))
    }
  }

  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    if(this.isEmpty) initial else xs.foldLeft(f(initial, x))(f)
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = {
    if(this.isEmpty) initial else f(x, xs.foldRight(initial)(f))
  }
    /*initial.append( f(x) )
    xs match{
      case MyNil() => initial.append(f(x))
      _ => MyCons[B](f(x), xs.foldRight()
    }*/

  def take(n: Int): MyList[A] = {
    if(n == 0) return MyNil()
    xs match{
      case MyNil() => MyCons[A](x, MyNil())
      case _ => MyCons[A](x, xs.take(n - 1))
    }
  }

  def drop(n: Int): MyList[A] = {
    if(n == 0) return MyCons[A](x, xs)
    xs match{
      case MyNil() => MyNil()
      case _ => xs.drop(n - 1)
    }
  ...
  def safeHead: Option[A] = Option(x)

  def safeTail: Option[MyList[A]] = Option(xs)
...
case class MyNil[A]() extends MyList[A] {
  ...
  def filter(pred: A => Boolean): MyList[A] = this
  ...
  def take(n: Int): MyList[A] = this

  def drop(n: Int): MyList[A] = this

  def head: A = throw new InvalidOperationException("head called on empty MyList")

  def tail: MyList[A] = throw new InvalidOperationException("tail called on empty MyList")

  def init: MyList[A] = throw new InvalidOperationException("init called on empty MyList")

  def last: A = throw new InvalidOperationException("last called on empty MyList")

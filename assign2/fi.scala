...
object Images {
  ...
  // !!FILL ME IN
  def lift0[A](value: A): Img[A] = pt => value
  ...
  // !!FILL ME IN
  def lift1[A, B](function: A => B)(image: Img[A]): Img[B] = pt => function(image(pt))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A,B,C](function: (A, B) => C)(ima: Img[A])(imb: Img[B]): Img[C] = pt => function(ima(pt), imb(pt))
  ...
  // !!FILL ME IN
  def lift3[A,B,C,D](function: (A, B, C) => D)(ima: Img[A])(imb: Img[B])(imc: Img[C]): Img[D] = pt => function(ima(pt), imb(pt), imc(pt))
  ...
  // colour images.
  // !!FILL ME IN
  val lerpImgClr: ImgGray=> ImgClr => ImgClr => ImgClr = gray => c1 => c2 => lift3(lerpClr)(gray)(c1)(c2)
  ...
  // !!FILL ME IN
  val overlayImgClr: ImgClr => ImgClr => ImgClr = c1 => c2 => lift2(overlayClr)(c1)(c2)
  ...
  // ImgMask argument.
  // !!FILL ME IN
  val selectImgClr: ImgMask => ImgClr => ImgClr => ImgClr = mask => ifTrue => ifFalse => pt => if(mask(pt)) ifTrue(pt) else ifFalse(pt)
  ...
  // !!FILL ME IN
  val msk2clr: ImgMask => ImgClr = mask => pt => if(mask(pt)) Colour(0, 0, 0, 1) else Colour(1, 1, 1, 1)
  ...
  // !!FILL ME IN
  val gray2clr: ImgGray => ImgClr = gray => pt => Colour(gray(pt), gray(pt), gray(pt), 1)
}
...
object SpatialTransforms {
  ...
  //Helper to find distance from origin
  def distance(p: Point): Double = math.sqrt((p.x * p.x) + (p.y * p.y))
  ...
  // y coordinate.
  // !!FILL ME IN
  def translatePt(v: Vector): TransformPt = pt => Point(pt.x + v.x, pt.y + v.y)
  ...
  // coordinate.
  // !!FILL ME IN
  def scalePt(v: Vector): TransformPt = pt => Point(pt.x * v.x, pt.y * v.y)
  ...
  // !!FILL ME IN
  def rotatePt(angle: Double): TransformPt = 
    pt => Point(pt.x * cos(angle) - pt.y * sin(angle), pt.y * cos(angle) + pt.x * sin(angle))
  ...
  // !!FILL ME IN
  def swirlPt(angle: Double): TransformPt = 
    pt => rotatePt(distance(pt) * (2 * math.Pi / angle))(pt)

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  def polarXf(polar: TransformPlr): TransformPt = 
    pt => plr2pt( polar( pt2plr(pt) ) )
  ...
  // !!FILL ME IN
  def translate[A](v: Vector): Filter[A] = img => pt => img( translatePt(v.neg)(pt) )
  ...
  // !!FILL ME IN
  def scale[A](v: Vector): Filter[A] = img => pt => img( scalePt(v.inverse)(pt) )
  ...
  // !!FILL ME IN
  def rotate[A](angle: Double): Filter[A] = img => pt => img(rotatePt(-angle)(pt))
  ...
  // !!FILL ME IN
  def swirl[A](angle: Double): Filter[A] = img => pt => img(swirlPt(-angle)(pt))
}
...
object Masking {
  ...
  // !!FILL ME IN
  val notM: ImgMask => ImgMask = lift1((b: Boolean) => if(!b) true else false)_
  ...
  // !!FILL ME IN
  val intersectM: ImgMask => ImgMask => ImgMask = lift2((a: Boolean, b: Boolean) => if(a && b) true else false)_
  ...
  // !!FILL ME IN
  val unionM: ImgMask => ImgMask => ImgMask = lift2((a: Boolean, b:Boolean) => if(a || b) true else false)_
  ...
  // !!FILL ME IN
  val xorM: ImgMask => ImgMask => ImgMask = lift2((a: Boolean, b: Boolean) => if(a ^ b) true else false)_
  ...
  // !!FILL ME IN
  def diffM(x: ImgMask)(y: ImgMask): ImgMask = pt => xorM(x)(y)(pt) && notM(y)(pt)
}

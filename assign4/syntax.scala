...
case class Construct( constructor:Label, typs:Seq[Type], e:Exp ) extends Exp
...
trait DebugStandardTokenParsers extends StdTokenParsers {
  class Wrap[+T](name:String,parser:Parser[T]) extends Parser[T] {
    def apply(in: Input): ParseResult[T] = {
      ...
      /*
      // For debugging purposes. Uncomment if you want to see parser creating the AST.
...
object Parsers extends StdTokenParsers with PackratParsers with DebugStandardTokenParsers {
  ...
  lazy val typedefs: Parser[Set[TypeDef]] =
    rep(typedef).flatMap(typedefs =>
      ifDuplicates(typedefs.map(_.name),
                   "type names",
                   typedefs).flatMap(typedefs =>
        ifDuplicates(typedefs.flatMap(_.constructors.keys),
                     "constructor names across types",
                     typedefs.toSet)))
  ...
  lazy val constructor: Parser[Exp] = tyname ~ opt(tyapp) ~  "(" ~ exp ~ ")" ^^ {
    case name ~ tvars ~ _ ~ arg ~ _ => tvars match {
      case None => Construct(name, Nil, arg)      
      case Some(ts) => Construct(name, ts, arg)  
    }} | tyname ~ opt(tyapp) ^^ { 
        case tname ~ tyvar => tyvar match { 
          case None => Construct(tname, Nil, Unit())
          case Some(tys) => Construct(tname, tys, Unit())
        }
    }
...
object Pretty {
  ...
  def prettyExp(e: Exp): String = e match {
    ...
    case Access(e, field) =>
      s"${prettyExp(e)}.$field"
    case Construct(constructor, tyvars, e) =>
      s"$constructor ${prettyExp(e)}"
    case Match(e, cases) =>

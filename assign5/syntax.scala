...
case class Construct( constructor:Label, e:Exp ) extends Exp
...
object Parsers extends StdTokenParsers with PackratParsers with DebugStandardTokenParsers {
  ...
  lazy val constructor: Parser[Exp] = tyname ~ "(" ~ exp ~ ")" ^^ {
    case name ~ _ ~ arg ~ _ => Construct(name, arg)
  } | tyname ^^ { case n => Construct(n, NilExp()) }
